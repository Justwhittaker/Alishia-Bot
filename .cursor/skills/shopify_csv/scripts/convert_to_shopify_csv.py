#!/usr/bin/env python3
"""Convert scraped product CSVs into Shopify Admin product-import CSVs.

Hard rules:
- Every product Status is forced to ``unlisted``
- Every product Published is forced to ``false``
- Missing required fields get Shopify-safe placeholders
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert scraped CSV data into a Shopify-friendly product import CSV (always unlisted)."
    )
    parser.add_argument("input_csv", type=Path, help="Path to scraped source CSV")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=None,
        help="Output Shopify CSV path (default: <input>_shopify_unlisted.csv)",
    )
    parser.add_argument(
        "--vendor",
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = args.output
    if output is None:
        output = input_csv.with_name(f"{input_csv.stem}_shopify_unlisted.csv")
